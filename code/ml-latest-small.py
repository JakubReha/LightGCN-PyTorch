...
import os
import math
data_path = "/Users/kuba/PycharmProjects/COMP4222/LightGCN-PyTorch/data/ml-latest-small/"
...
with open(os.path.join(data_path, "all_data_multiplied.txt"), 'w') as fp:
...
with open(os.path.join(data_path, "test_multiplied.txt"), 'w') as fp:
    for i, item in enumerate(new.values):
        fp.write("%s" % str(i))
        split = int(np.ceil(0.8*len(item)))
